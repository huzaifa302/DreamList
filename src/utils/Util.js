...
function  maskEmail(email) {
  ...
  if (index <= 2 || index == -1) return email;

  const visiblePart = email.slice(0, 2) 

  const maskPart = email.slice(2, index).replace(/./g, '*')

  return visiblePart + maskPart + email.slice(index)
}
...
function tabbar(itemId , currentId){
  let stepsNameColor;
  let bordersColor;
  ...
  if (itemId < currentId) {
    stepsNameColor=Colors.black
    bordersColor= Colors.PRIMARY_COLOR;
    stepbackgroundColor= Colors.PRIMARY_COLOR
    stepNumberColor= Colors.white;
  }
  else if (itemId === currentId) {
    stepsNameColor= Colors.PRIMARY_COLOR
    bordersColor= Colors.PRIMARY_COLOR
    stepbackgroundColor= Colors.white
    stepNumberColor= Colors.PRIMARY_COLOR
  }
  else {
    stepsNameColor= Colors.LIGHT_TONE
    bordersColor= Colors.LIGHT_TONE
    stepbackgroundColor= Colors.LIGHT_TONE
    stepNumberColor= Colors.white
  }
return {
  stepsNameColor,
